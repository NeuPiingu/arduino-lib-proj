  ...
  class CameraControls extends EventDispatcher {
    constructor(camera, domElement) {
      ...
      this._rotateInternal = (deltaX, deltaY) => {
        const theta =
          (PI_2 * this.azimuthRotateSpeed * deltaX) / this._elementRect.height;
        const phi =
      ...
      this._dollyInternal = (delta, x, y) => {
        ...
        if (
          ...
          this._targetEnd.add(
            _v3A.normalize().multiplyScalar(signedPrevRadius)
          );
        ...
        const onPointerDown = (event) => {
          ...
          switch (event.button) {
            case THREE.MOUSE.LEFT:
              this._state = event.shiftKey
                ? this.mouseButtons.shiftLeft
    ...
    getDistanceToFit(width, height, depth) {
      console.warn(
        "camera-controls: getDistanceToFit() has been renamed to getDistanceToFitBox()"
      );
    ...
    update(delta) {
      ...
      const deltaOffset = _v3B.subVectors(
      ...
      if (this._dollyControlAmount !== 0) {
        if (isPerspectiveCamera(this._camera)) {
          ...
          const worldToScreen =
            this._sphericalEnd.radius *
            Math.tan(camera.getEffectiveFOV() * THREE.MathUtils.DEG2RAD * 0.5);
          const prevRadius =
            this._sphericalEnd.radius - this._dollyControlAmount;
          const lerpRatio =
          ...
          const cursor = _v3C
            .copy(worldPosition)
            .add(quaternion.multiplyScalar(distance));
          this._targetEnd.lerp(
            cursor,
            1 - camera.zoom / this._dollyControlAmount
          );
          this._target.copy(this._targetEnd);
        }
        this._dollyControlAmount = 0;
    ...
    fromJSON(json, enableTransition = false) {
      ...
      _sphericalA.setFromVector3(
        position.sub(this._targetEnd).applyQuaternion(this._yAxisUpSpace)
      );
